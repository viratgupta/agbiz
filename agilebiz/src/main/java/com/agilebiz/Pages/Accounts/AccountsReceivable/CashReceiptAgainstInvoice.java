 ...
 *
 */
public class CashReceiptAgainstInvoice extends TestBase {
	...
	public String VerifyCashReceiptAgainstInvoicePage(String PageName) throws Exception {
		try {
			driverwait(2);
			...
			if (!headerValue.equals("Cash Receipt Against Invoice")) {
				throw new SkipException("Cash Receipt Against Invoice Page couldn't open.");
			}
		} catch (Exception ex) {
	...
	public boolean addValueToBranchTextBox(String branch) {
		try {
			WebElement branchTextBoxWebEelement = new WebDriverWait(driver, 200)
					.until(ExpectedConditions.visibilityOf(getWebElement("AR_CRAI_branch")));
	...
	public boolean selectdocdate() {
		try {
			WebElement docdateWebEelement = new WebDriverWait(driver, 200)
					.until(ExpectedConditions.visibilityOf(getWebElement("AR_CRAI_docdate")));
	...
	public boolean addValueTocustomerTextBox(String customer) {
		try {
			WebElement custTextBoxWebEelement = new WebDriverWait(driver, 200)
					.until(ExpectedConditions.visibilityOf(getWebElement("AR_CRAI_customer")));
	...
	public boolean addValueTocashaccTextBox(String cashacc) {
		try {
			WebElement cashaccTextBoxWebEelement = new WebDriverWait(driver, 200)
					.until(ExpectedConditions.visibilityOf(getWebElement("AR_CRAI_cashac")));
			cashaccTextBoxWebEelement.clear();
			cashaccTextBoxWebEelement.sendKeys(cashacc);
			webElementWait(getWebElement("autcompletedd"));
			cashaccTextBoxWebEelement.sendKeys(Keys.TAB);
	...
	public boolean addValueTocurrencyReceiptAmtTextBox(String receiptamt) {
		try {
			WebElement recTextBoxWebEelement = new WebDriverWait(driver, 200)
					.until(ExpectedConditions.visibilityOf(getWebElement("AR_CRAI_receiptamt")));
	...
	public boolean addValueToTDSTextBox(String tdsvalue) {
		try {
			WebElement tdsTextBoxWebEelement = new WebDriverWait(driver, 200)
					.until(ExpectedConditions.visibilityOf(getWebElement("AR_CRAI_tdsamt")));
	...
	public boolean addValueToRefNoTextBox(String refno) {
		try {
			WebElement refnoTextBoxWebEelement = new WebDriverWait(driver, 200)
					.until(ExpectedConditions.visibilityOf(getWebElement("AR_CRAI_refno")));
	...
	public boolean itemDetailsfromFillgrid(String billno) throws Exception
	{
		...
		try {
			driverwait(2);
			getWebElement("AR_CRAI_listofinovoicefill").click();
			if(isElementPresent("AR_CRAI_fillnodata"))
			{
				driverwait(2);
				getWebElement("AR_CRAI_fillOKbtn").click();
				Reporter.log("No Record Found In Fillgrid");
				return false;
			}
			else{
				WebElement fillgridcheckall = new WebDriverWait(driver, 100)
						.until(ExpectedConditions.visibilityOf(getWebElement("AR_CRAI_fillsearch")));
				fillgridcheckall.clear();
				fillgridcheckall.sendKeys(billno);
				driverwait(2);
				getWebElement("AR_CRAI_fillgridchechkall").click();
				getWebElement("AR_CRAI_fillOKbtn").click();
				driverwait(2);
				return true;
			}
