...
public class SalesJournalEntry extends TestBase {
	...
	public String VerifySalesJournalEntryPage(String PageName) throws Exception {
		try {
			driverwait(2);
			...
			if (!headerValue.equals("Sales Journal Entry")) {
				throw new SkipException("Sales Journal Entry Page couldn't open.");
			}
		} catch (Exception ex) {
	...
	public boolean addValueToBranchTextBox(String branch) {
		try {
			WebElement branchTextBoxWebEelement = new WebDriverWait(driver, 200)
					.until(ExpectedConditions.visibilityOf(getWebElement("AR_SJE_branch")));
	...
	public boolean selectdocdate() {
		try {
			WebElement docdateWebEelement = new WebDriverWait(driver, 200)
					.until(ExpectedConditions.visibilityOf(getWebElement("AR_SJE_docdate")));
	...
	public boolean addValueTocustomerTextBox(String customer) {
		try {
			WebElement custTextBoxWebEelement = new WebDriverWait(driver, 200)
					.until(ExpectedConditions.visibilityOf(getWebElement("AR_SJE_customer")));
	...
	public boolean addValueTocurrencyTextBox(String currency) {
		try {
			WebElement currTextBoxWebEelement = new WebDriverWait(driver, 200)
					.until(ExpectedConditions.visibilityOf(getWebElement("AR_SJE_currency")));
	...
	public boolean addValueToRefNoTextBox(String refno) {
		try {
			WebElement refnoTextBoxWebEelement = new WebDriverWait(driver, 200)
					.until(ExpectedConditions.visibilityOf(getWebElement("AR_SJE_refno")));
	...
	public boolean addValueToRefdateTextBox(String refdate) {
		try {
			WebElement refdateTextBoxWebEelement = new WebDriverWait(driver, 200)
					.until(ExpectedConditions.visibilityOf(getWebElement("AR_SJE_refdate")));
